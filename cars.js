...
function ferraris(array) {
    ...
    for(i = 0; i < array.length; i ++) {
        let element = array[i];
        if (element.brand === 'Ferrari') {
            ferraris.push(element);
        }
    }
    return ferraris;
}
...
function list(array) {
    const newArray = array.map(function(x) {
        return `This ${x.brand} from ${x.model} costs $${x.price}`;
    })
    return newArray;
}
...
function bmwSum(array) {
    const _BMW = array.filter(function(x) {
        return x.brand === 'BMW';
    })

    const values = _BMW.map(function(x) {
        return Object.values(x);
    })

    const prices = values.map(function(x) {
        return x[1];
    })
